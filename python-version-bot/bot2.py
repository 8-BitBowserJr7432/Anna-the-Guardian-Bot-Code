...
async def start(ctx):
    ...
    message = (
        ...
        "INSTALLING PACKAGES :loading_bg~1:  …\n"
...
async def copyEmoji(ctx, emoji: str): # changed to str to prevent conversion errors
    ...
    try:
        ...
        if not partial_emoji.is_custom_emoji():
            return await ctx.send("❌ You can only copy **Custom Emojis**, not standard ones like 😀.")
...
async def copy_emoji_slash(interaction: discord.Interaction, emoji: str):
    ...
    try:
        ...
        await interaction.followup.send(f"✅ Steal successful! Added {new_emoji}")
...
async def give_role(ctx, target: str, role: discord.Role):
    ...
    if ctx.guild.me.top_role <= role:
        return await ctx.send("❌ My role is not high enough to assign this role!")

    # --- MODE: ALL ---
    if target.lower() == "all":
        status_msg = await ctx.send(f"⏳ Starting mass-role for **all** members. This may take a while...")
        count = 0
        ...
        await status_msg.edit(content=f"✅ Finished! Added {role.name} to **{count}** members.")
    ...
    # --- MODE: SINGLE MEMBER ---
        ...
        try:
            ...
            await ctx.send(f"❌ Could not find a member named `{target}`. Use a mention, ID, or type `all`.")
